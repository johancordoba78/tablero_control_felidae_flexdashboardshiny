...
-----------------------------------------------------------------------
```{r Filtros}
selectInput(input = "especie" , label="Especie", choices =
              lista_especies, selected = "Todas")

selectInput(input = "provincia" , label="Provincia", choices = lista_provincias, selected = "Todas")
...
# Filtrado de felidae por especies
  ...
  # Filtrado de felidae por fecha
    felidae_filtrado <-
      felidae_filtrado %>%
      filter(eventDate >= as.Date(input$fecha[1], origin ="1970-01-01") & eventDate <=
               as.Date(input$fecha[2], origin ="1970-01-01"))
...
leaflet() %>%
  ...
  addCircleMarkers(
    ...
    popup = paste0(
      ...
      registros$eventDate,
      "<br>",
      '<a href=">', registros$ocurrenceID, ' ">', "Más información",'</a>'
    )
